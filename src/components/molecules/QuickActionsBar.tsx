...
export default function QuickActionsBar({ opportunityId, clientId, clientName, clientEmail, onCreateTask }: QuickActionsBarProps) {
  ...
  return (
    <>
      ...
      {isEmailModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md" onClick={() => setIsEmailModalOpen(false)}>
          <div className="bg-[#0f0f0f]/98 backdrop-blur-sm border border-white/20 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
